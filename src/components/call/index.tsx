...
export default function CallSection({close, roomId}: { close: Function, roomId: string; }) {
    const servers = {
        iceServers: [
            {
    ...
    const micButton = () => {
        ...
        if (audioON) {
            ...
            >
                <rect x="9" y="3" width="6" height="10" rx="3" stroke="red" strokeWidth="2"/>
                <path
                    ...
                    strokeLinecap="round"
                />
                <path d="M12 17V21M12 21H7M12 21H17" stroke="red" strokeWidth="2" strokeLinecap="round"/>
            </svg>
        } else {
            icon = <svg
                width="24"
        ...
        return <button onClick={() => setAudioON(prev => !prev)}>{icon}</button>
    }
    ...
    const videoButton = () => {
        ...
        return <button onClick={() => setVideoON(prev => !prev)}>{icon}</button>
    }
    const endcallButton = () => {
    ...
    useEffect(() => {
        if (videoON) {
            ...
            navigator.mediaDevices.getUserMedia({audio: audioON, video: videoON}).then((stream) => {
                if (ref.current) {
            ...
            {makeCall &&
                <div className="video-list">
                    <div className="video-container">
                        <video ref={ref} id="video1" autoPlay/>
                    </div>
                    <div className="video-container">
                        <video ref={ref2} id="video2" autoPlay/>
                    </div>
